                      ...
                      DWORD dwClsContext, LPVOID lpvReserved, void **ppv)
{
    ...
    if ((!szProgId && !szPathName) || !riid || !ppv || lpvReserved)
        return DH_EXIT(E_INVALIDARG, szProgId);
...
HRESULT dhGetObject(LPCOLESTR szPathName, LPCOLESTR szProgId, IDispatch **ppDisp)
{
    ...
    return DH_EXIT(dhGetObjectEx(szPathName, szProgId, &IID_IDispatch,
                                 CLSCTX_LOCAL_SERVER | CLSCTX_INPROC_SERVER, NULL, (void **)ppDisp),
                   szProgId);
}
...
static HRESULT TraverseSubObjects(IDispatch **ppDisp, LPWSTR *lpszMember, va_list *marker)
{
    ...
    do
    {
        *szSeperator = L'\0';
        ...
        if (!vtObject.pdispVal && SUCCEEDED(hr))
            hr = E_NOINTERFACE;
...
static HRESULT ExtractArgument(VARIANT *pvArg, WCHAR chIdentifier, BOOL *pbFreeArg, va_list *marker)
{
    ...
    *pbFreeArg = FALSE;
    ...
    case L'S':
    {
        ...
        if (V_BSTR(pvArg) == NULL && szTemp != NULL)
            hr = E_OUTOFMEMORY;
...
HRESULT ConvertVariantTimeToFileTime(DATE date, FILETIME *pft)
{
    ...
    if (date < VARIANT_FILE_TIME_DAY0)
        return E_INVALIDARG;
...
HRESULT ConvertVariantTimeToSystemTime(DATE date, SYSTEMTIME *pSystemTime)
{
    ...
    if (!pSystemTime)
        return E_INVALIDARG;
    if (FAILED(hr = ConvertVariantTimeToFileTime(date, &fileTime)))
        return hr;
...
HRESULT ConvertTimeTToVariantTime(time_t timeT, DATE *pDate)
{
    ...
    if ((ptm = localtime(&timeT)) == NULL)
        return E_FAIL;
    localtm = *ptm;

    if ((ptm = gmtime(&timeT)) == NULL)
        return E_FAIL;
    utctm = *ptm;
...
#define CheckTlsInitialized() \
    if (f_lngTlsInitEnd != 0) \
                 ...
                 EXCEPINFO *pExcepInfo, UINT iArgError, LPCWSTR szFunctionName)
{
    ...
    if (FAILED(hr) && !g_ExceptionOptions.bDisableRecordExceptions)
    {
        ...
        if (pException->hr == 0)
        {
            ...
            if (pExcepInfo && hr == DISP_E_EXCEPTION)
            {
                if (pExcepInfo->pfnDeferredFillIn &&
                    !IsBadCodePtr((FARPROC)pExcepInfo->pfnDeferredFillIn))
        ...
        if (nStackCount == 1)
        {
            ...
            if (szCompleteMember)
                hlprStringCchCopyW(pException->szCompleteMember, ARRAYSIZE(pException->szCompleteMember), szCompleteMember);
...
#define DESCRIPTION_LENGTH 255
    ...
    hr = (pException->hr == DISP_E_EXCEPTION && pException->swCode ? pException->swCode : pException->hr);
