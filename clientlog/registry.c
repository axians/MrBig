...
static const registry_Item RegistryDefaultItems[] = {
    {HKEY_LOCAL_MACHINE, "HKLM", "SOFTWARE\\Axians\\ComputerProperties",
     "Customer"},
...
static BOOL registry_ItemsEqual(const registry_Item *a,
    ...
    if (a->root != b->root)
        return FALSE;
    if (a->subkey == NULL || b->subkey == NULL || a->valueName == NULL ||
        b->valueName == NULL)
        return FALSE;
    return _stricmp(a->subkey, b->subkey) == 0 &&
           _stricmp(a->valueName, b->valueName) == 0;
}

static BOOL registry_ContainsItem(const registry_ConfigItem *items,
                                  size_t numItems,
                                  const registry_Item *candidate) {
...
static VOID registry_AddDefaultItems(registry_ConfigItem *items,
                                     size_t *numItems) {
    ...
    for (size_t i = 0; i < lengthof(RegistryDefaultItems); i++) {
        if (*numItems >= REGISTRY_MAX_CONFIG_ITEMS) {
            LOG_DEBUG("\tregistry.c: Reached max config items while adding "
                      "defaults.");
            return;
        }
        ...
        snprintf(dst->Subkey, sizeof(dst->Subkey), "%s",
                 RegistryDefaultItems[i].subkey);
        snprintf(dst->ValueName, sizeof(dst->ValueName), "%s",
                 RegistryDefaultItems[i].valueName);
...
static BOOL registry_ParseRoot(LPCSTR root, HKEY *hRoot, LPCSTR *rootName) {
    if (root == NULL || hRoot == NULL || rootName == NULL)
        return FALSE;

    if (_stricmp(root, "HKLM") == 0 ||
        _stricmp(root, "HKEY_LOCAL_MACHINE") == 0) {
        *hRoot = HKEY_LOCAL_MACHINE;
        *rootName = "HKLM";
        return TRUE;
    }
    if (_stricmp(root, "HKCU") == 0 ||
        _stricmp(root, "HKEY_CURRENT_USER") == 0) {
        *hRoot = HKEY_CURRENT_USER;
        *rootName = "HKCU";
        return TRUE;
    }
    if (_stricmp(root, "HKCR") == 0 ||
        _stricmp(root, "HKEY_CLASSES_ROOT") == 0) {
    ...
    if (_stricmp(root, "HKCC") == 0 ||
        _stricmp(root, "HKEY_CURRENT_CONFIG") == 0) {
...
static BOOL registry_AddItemFromPath(const CHAR *line,
    ...
    if (line == NULL || items == NULL || numItems == NULL)
        return FALSE;
    if (*numItems >= REGISTRY_MAX_CONFIG_ITEMS)
        return FALSE;
...
static VOID registry_GetConfiguredItems(registry_ConfigItem *items,
    ...
    *numItems = 0;
    ...
    LOG_DEBUG("\tregistry.c: Using %llu registry item(s) (defaults included), "
              "config source '%s'.",
...
static BOOL registry_TryReadAndAppendValue(const registry_Item *item,
                                           clog_Arena *scratch) {
    DWORD type = 0;
    DWORD cbData = 0;

    if (item == NULL || item->subkey == NULL || item->valueName == NULL ||
        scratch == NULL || scratch->State == NULL) {
        LOG_DEBUG(
            "\t\tregistry.c: Invalid input to registry_TryReadAndAppendValue.");
        return FALSE;
    }

    LOG_DEBUG("\t\tregistry.c: Reading %s\\%s\\%s.", item->rootName,
              item->subkey, item->valueName);

    LONG status = RegGetValueA(item->root, item->subkey, item->valueName,
                               RRF_RT_ANY, &type, NULL, &cbData);

    if (status != ERROR_SUCCESS || cbData == 0 || cbData > REG_VALUE_BUF_SIZE) {
        LOG_DEBUG("\t\tregistry.c: Value missing or unsupported for %s\\%s\\%s "
                  "(status=%ld, size=%lu).",
                  item->rootName, item->subkey, item->valueName, status,
                  cbData);
        return FALSE;
    }
    ...
    case REG_SZ:
    case REG_EXPAND_SZ: {
        status = RegGetValueA(item->root, item->subkey, item->valueName,
                              (type == REG_SZ) ? RRF_RT_REG_SZ
                                               : RRF_RT_REG_EXPAND_SZ,
                              NULL, valueBuf, &outSize);
        if (status != ERROR_SUCCESS || outSize == 0) {
            LOG_DEBUG(
                "\t\tregistry.c: Failed reading string value for %s\\%s\\%s "
                "(status=%ld, size=%lu).",
                item->rootName, item->subkey, item->valueName, status, outSize);
            return FALSE;
        }
    ...
    case REG_MULTI_SZ: {
        status = RegGetValueA(item->root, item->subkey, item->valueName,
                              RRF_RT_REG_MULTI_SZ, NULL, valueBuf, &outSize);
        if (status != ERROR_SUCCESS || outSize < 2) {
            LOG_DEBUG("\t\tregistry.c: Failed reading multi-string value for "
                      "%s\\%s\\%s (status=%ld, size=%lu).",
                      item->rootName, item->subkey, item->valueName, status,
                      outSize);
            return FALSE;
        }
    ...
    case REG_DWORD: {
        ...
        status = RegGetValueA(item->root, item->subkey, item->valueName,
                              RRF_RT_REG_DWORD, NULL, &value, &outSize);
        if (status != ERROR_SUCCESS || outSize != sizeof(value)) {
            LOG_DEBUG(
                "\t\tregistry.c: Failed reading DWORD value for %s\\%s\\%s "
                "(status=%ld, size=%lu).",
                item->rootName, item->subkey, item->valueName, status, outSize);
            return FALSE;
        }
    ...
    case REG_QWORD: {
        ...
        outSize = sizeof(value);
        status = RegGetValueA(item->root, item->subkey, item->valueName,
                              RRF_RT_REG_QWORD, NULL, &value, &outSize);
        if (status != ERROR_SUCCESS || outSize != sizeof(value)) {
            LOG_DEBUG(
                "\t\tregistry.c: Failed reading QWORD value for %s\\%s\\%s "
                "(status=%ld, size=%lu).",
                item->rootName, item->subkey, item->valueName, status, outSize);
            return FALSE;
        }
    ...
    case REG_BINARY: {
        status = RegGetValueA(item->root, item->subkey, item->valueName,
                              RRF_RT_REG_BINARY, NULL, valueBuf, &outSize);
        if (status != ERROR_SUCCESS || outSize == 0) {
            LOG_DEBUG(
                "\t\tregistry.c: Failed reading binary value for %s\\%s\\%s "
                "(status=%ld, size=%lu).",
                item->rootName, item->subkey, item->valueName, status, outSize);
            return FALSE;
        }
        ...
        size_t written = 0;
        for (size_t i = 0; i < toPrint && written + 3 < sizeof(prettyValue);
             i++) {
            int n =
                snprintf(&prettyValue[written], sizeof(prettyValue) - written,
                         "%02X", valueBuf[i]);
    ...
    default:
        LOG_DEBUG(
            "\t\tregistry.c: Unsupported registry type %lu for %s\\%s\\%s.",
            type, item->rootName, item->subkey, item->valueName);
        return FALSE;
    }
    ...
    clog_ArenaAppend(scratch, "\n%s\\%s\\%s:\t%s", item->rootName, item->subkey,
                     item->valueName, prettyValue);
    LOG_DEBUG("\t\tregistry.c: Read succeeded for %s\\%s\\%s.", item->rootName,
              item->subkey, item->valueName);
    return TRUE;
}
...
void clog_registry(clog_Arena scratch) {
    ...
    registry_GetConfiguredItems(configuredItems, &numItems, configSource,
                                sizeof(configSource));

    LOG_DEBUG("\tregistry.c: Config source '%s'.",
              configSource[0] ? configSource : "unknown");
    LOG_DEBUG("\tregistry.c: Starting registry read pass with %llu item(s).",
              (unsigned long long)numItems);

    if (scratch.State == NULL) {
        LOG_DEBUG(
            "\tregistry.c: Arena state is NULL, aborting registry output to "
            "avoid crash.");
        return;
    }
    ...
    for (size_t i = 0; i < numItems; i++) {
        LOG_DEBUG(
            "\t\t[%llu] Root='%s' Path='%s' Subvalue='%s'",
            (unsigned long long)(i + 1),
            configuredItems[i].Item.rootName ? configuredItems[i].Item.rootName
                                             : "(null)",
            configuredItems[i].Item.subkey ? configuredItems[i].Item.subkey
                                           : "(null)",
            configuredItems[i].Item.valueName
                ? configuredItems[i].Item.valueName
                : "(null)");
    }
    ...
    for (size_t i = 0; i < numItems; i++) {
        numAttempted++;
        if (registry_TryReadAndAppendValue(&configuredItems[i].Item,
                                           &scratch)) {
            numSucceeded++;
        }
    }
