...
green Tue Jul 06 02:43:47 VS 2004 [ntserver] up: 3 days, 1 users, 16 procs,
load=1%, PhysicalMem: 64MB(53%)
...
static long get_uptime(void) {
    ...
    if (debug > 1)
        mrlog("get_uptime returns %ld", result);
    return result;
}
...
void cpu(void) {
...

    {
        ...
        snprintf(since, sizeof(since), "%04u-%02u-%02u %02u:%02u:%02u",
                 (unsigned)(boottime.wYear > 9999 ? 9999 : boottime.wYear),
                 (unsigned)(boottime.wMonth > 12 ? 12 : boottime.wMonth),
                 (unsigned)(boottime.wDay > 31 ? 31 : boottime.wDay),
                 (unsigned)(boottime.wHour > 23 ? 23 : boottime.wHour),
                 (unsigned)(boottime.wMinute > 59 ? 59 : boottime.wMinute),
                 (unsigned)(boottime.wSecond > 59 ? 59 : boottime.wSecond));
    }
    uc = users();
    load = 0;
    pc = pscount();
    ...
    b[0] = '\0';
    snprcat(b, n,
            "%s up: %s, %d users, %d procs, load=%d%%, PhysicalMem: %ldMB "
            "(%d%%)\n%s\n"
            "Memory Statistics\n"
            ...
            "Windows version %d.%d\n"
            "%s %s (%s)\n",
            now, up, uc, pc, load, (long)(statex.ullTotalPhys / (1024 * 1024)),
            (int)memusage, r,
#if 0
...
#else
            WIDTH, (double)statex.ullTotalPhys, WIDTH,
            (double)statex.ullAvailPhys, WIDTH, (double)statex.ullTotalPageFile,
            WIDTH, (double)statex.ullAvailPageFile, WIDTH,
            (double)statex.ullTotalVirtual, WIDTH,
            (double)statex.ullAvailVirtual,
#endif
            up, since, (int)osvi.dwMajorVersion, (int)osvi.dwMinorVersion,
            PACKAGE, VERSION, CPU);
    append_limits(b, n);
