...
static PERF_OBJECT_TYPE *get_object(PERF_DATA_BLOCK *bp, int object)
{
    ...
    if (debug > 1)
        mrlog("get_object(%p, %d)", bp, object);

    op = first_object(bp);
    for (a = 0; a < bp->NumObjectTypes; a++)
    {
        if (debug > 1)
            mrlog("object %d is %d", a,
                  op->ObjectNameTitleIndex);
        if (op->ObjectNameTitleIndex == object)
            return op;
        op = next_object(op);
    }
    return NULL;
                                      ...
                                      long long *perf_time, long long *perf_freq)
{
    ...
    if (debug)
        mrlog("read_perfcounters(object = %ld)", (long)object);
    ...
    if (debug > 1)
        mrlog("%d interesting counters", ncounters);

    obj[0] = '\0';
    snprcat(obj, sizeof obj, "%ld", (long)object);
    if (debug > 2)
        mrlog("About to call RegQueryValueEx");
    ret = RegQueryValueEx(HKEY_PERFORMANCE_DATA, obj, 0, &type,
                          (BYTE *)data, &size);
    if (debug > 2)
        mrlog("RegQueryValueEx returned %ld", ret);
    while (ret != ERROR_SUCCESS)
    {
        if (ret == ERROR_MORE_DATA)
        {
            if (debug > 1)
                mrlog("Increase buffer");
    ...
    if (debug > 1)
        mrlog("RegQueryValueEx returned ERROR_SUCCESS");
    object_ptr = get_object(data, object);
    if (debug > 2)
        mrlog("get_object returned %p", object_ptr);
    if (object_ptr == NULL)
    {
    ...
    if (perf_time)
        *perf_time = object_ptr->PerfTime.QuadPart;
    if (perf_freq)
        *perf_freq = object_ptr->PerfFreq.QuadPart;
...
Done:
    if (debug > 1)
        mrlog("read_perfcounters returns %p", results);
    big_free("read_perfcounters (data)", data);
...
void print_perfcounters(struct perfcounter *pc, int ncounters)
{
    ...
    if (debug > 1)
        mrlog("print_perfcounters(%p, %d)", pc, ncounters);
