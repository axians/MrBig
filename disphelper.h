...
#define SAFE_FREE_STRING(string) \
    {                            \
        dhFreeString(string);    \
        (string) = NULL;         \
    }
...
#define DH_ENTER(szFunctionName)                          \
    static LPCWSTR xx_szFunctionName_xx = szFunctionName; \
    dhEnter()
#else
#define DH_ENTER(szFunctionName)                          \
    static LPCWSTR xx_szFunctionName_xx = szFunctionName; \
    fwprintf(stderr, L"DH_ENTER(%s)\n", szFunctionName);  \
    dhEnter()
#endif
...
#define DH_ENTER(szFunctionName)
#define DH_EXITEX(hr, bDispatchError, szMember, szCompleteMember, pExcepInfo, iArgError)                     \
    (((hr == DISP_E_EXCEPTION && pExcepInfo) ? (SysFreeString(((EXCEPINFO *)(pExcepInfo))->bstrSource),      \
                                                SysFreeString(((EXCEPINFO *)(pExcepInfo))->bstrDescription), \
                                                SysFreeString(((EXCEPINFO *)(pExcepInfo))->bstrHelpFile), 0) \
                                             : (0)),                                                         \
     hr)
...
{
    if (FAILED(hr))
        dhThrowFunctions::throw_wstring();
    return true;
}
...
{
    if (FAILED(hr))
        dhThrowFunctions::throw_dhexception();
    return true;
}
