...
static void store_chunk(void *p, size_t n, char *cl)
{
    ...
    if (debug >= 3)
        mrlog("Storing chunk %p (%s) of %ld bytes in slot %d",
              p, cl, (long)n, i);
    chunks[i].p = p;
    chunks[i].n = n;
...
static void remove_chunk(void *p, char *cl)
{
    ...
    if (check_chunk(i))
        mrexit("remove_chunk: check not ok", EXIT_FAILURE);
    if (debug >= 3)
    {
...
void big_free(char *p, void *q)
{
    ...
    {
        if (debug)
            mrlog("No need to free %p on behalf of %s", q, p);
        return;
    }
    if (debug > 2)
        mrlog("Freeing %p on behalf of %s", q, p);
...
int big_fclose(char *p, FILE *fp)
{
    int n = fclose(fp);
    if (debug > 1)
        mrlog("Closing %p on behalf of %s", fp, p);
    if (n)
    {
...
char *get_option(char *n, int partial)
{
    ...
    {
        if ((partial && !strncmp(n, o->name, l)) || !strcmp(n, o->name))
            return o->name;
    }
    return NULL;
}
...
static void readcfg(void)
{
    ...
    pickupdir[0] = '\0';
    if (logfp)
        big_fclose("readcfg:logfile", logfp);
    logfp = NULL;
...
static int insert_status(char *machine, char *test, char *color)
{
    ...
    if (debug > 1)
        mrlog("insert_status(%s, %s, %s)", machine, test, color);
    for (s = teststatus; s; s = s->next)
    {
        ...
        {
            if (debug)
                mrlog("insert_status found match");
            break;
        }
    }
    if (s == NULL)
    ...
    if (!strcmp(color, s->color))
    {
        ...
        if (now - s->last >= mrsleep)
        {
            ...
            */
            if (debug)
                mrlog("insert_status: mrsleep exceeded");
            s->last = now;
            s->grace = 0;
            return 2;
        }
...
void mrsend(char *machine, char *test, char *color, char *message)
{
    ...
    if (debug > 1)
        mrlog("mrsend(%s, %s, %s, %s)", machine, test, color, message);

    is = insert_status(machine, test, color);
    if (is == 0)
    {
        if (debug)
            mrlog("mrsend: no change, nothing to do");
        return;
    }
...
{
    if (debug > 1)
        mrlog("mrsend_clientlog(%s, ...)", machine);
    send_update(message);
}
...
void mrbig(void)
{
    ...
    for (;;)
    {
        ...
        if (pickupdir[0])
            ext_tests(is_ext_filter_enabled);
        ...
        if (sleeptime < SLEEP_MIN)
            sleeptime = SLEEP_MIN;
        if (debug)
            mrlog("started at %d, finished at %d, sleep for %d",
