...
int contains_case_insensitive(const char *str, const char *substr)
{
    ...
    if (len_str < len_sub)
        return 0; // can't contain a longer substring
...
int is_allowed_ext(const char *cmd)
{
    ...
    char basepath[1024];
    if (get_basepath(cmd, basepath, sizeof(basepath)) == NULL)
        return 0;

    if (debug)
        mrlog("is_allowed_ext: ext='%s', basepath='%s'\n", ext, basepath);
...
void ext_tests(int is_filter_enabled)
{
    ...
    for (i = 0; get_cfg("ext", cmd, sizeof cmd, i); i++)
    {
        ...
        if (cmd[0] == '#' || cmd[0] == '\0')
            continue;
        if (debug)
            mrlog("Ext test: %s", cmd);
        if (is_filter_enabled && !is_allowed_ext(cmd))
        {
            if (debug)
                mrlog("Skipping disallowed ext test: %s", cmd);
            continue;
        }
                          ...
                          NULL, NULL, &si, &pi))
        {
            ...
            if (debug)
                mrlog("WaitForSingleObject returns %d", n);
            if (n == WAIT_TIMEOUT)
            {
