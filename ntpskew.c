...
static int get_ntp_servers(char servers[NTP_MAX_SERVERS][NTP_SERVER_LEN])
{
    ...
    cbData = sizeof ntpType; RegQueryValueExA(hKey, "Type",      NULL, NULL, (LPBYTE)ntpType, &cbData);
    cbData = sizeof raw;     RegQueryValueExA(hKey, "NtpServer", NULL, NULL, (LPBYTE)raw,     &cbData);
    RegCloseKey(hKey);
    ...
    if (!strcmp(ntpType, "NT5DS")) {
        ...
        for (tok = strtok(raw, " \t"); tok && n < NTP_MAX_SERVERS; tok = strtok(NULL, " \t")) {
            if ((comma = strchr(tok, ','))) *comma = '\0';
            if (*tok) strlcpy(servers[n++], tok, NTP_SERVER_LEN);
        }
...
static int stripchart_offset(const char *server, double *offset)
{
    ...
    *offset = 0.0;
    ...
    if (!(fp = _popen(cmd, "r"))) return 0;

    while (fgets(line, sizeof line, fp)) {
        if (line[2] == ':' && line[5] == ':') {          /* HH:MM:SS, ... */
            char *p = strstr(line, "o:");                 /* prefer clock offset */
            if (!p) { p = strchr(line, ','); if (p) p++; }
            else p += 2;
            while (p && *p == ' ') p++;
            if (p && sscanf(p, "%lf", offset) == 1) found = 1;
        }
    }
    _pclose(fp);
    return found;
...
{
    if (!strcmp(a,"red") || !strcmp(b,"red")) return "red";
    if (!strcmp(a,"yellow") || !strcmp(b,"yellow")) return "yellow";
    return "green";
}

static const char *color_for(double abs_val, int yellow_s, int red_s)
{
    if (abs_val >= red_s)    return "red";
    if (abs_val >= yellow_s) return "yellow";
    return "green";
}
                                           ...
                                           int yellow_s, int red_s)
{
    ...
    *pos += snprintf(msg + *pos, msgsz - *pos,
    ...
    for (i = 0; i < n && *pos < (int)msgsz - 512; i++) {
                    ...
                    if (wb) {
                        if (EvtFormatMessage(pm, events[i], 0,0,NULL,
                            ...
                            while (wb[wi] && ni < sizeof msgtext - 1) {
                                wchar_t wc = wb[wi++];
                                if (wc >= 0x20 && wc <= 0x7E) msgtext[ni++] = (char)wc;
                            }
                            msgtext[ni] = '\0';
        ...
        long diff = -1;
        if (t_new) { struct tm *g=gmtime(&t_new); strftime(ts_new,sizeof ts_new,"%Y-%m-%d %H:%M:%S",g); }
        if (t_old) { struct tm *g=gmtime(&t_old); strftime(ts_old,sizeof ts_old,"%Y-%m-%d %H:%M:%S",g); }
        if (t_new && t_old) diff = (long)labs((long)(t_new - t_old));
